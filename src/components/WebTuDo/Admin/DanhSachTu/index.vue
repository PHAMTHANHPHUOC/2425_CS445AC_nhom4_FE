  ...
  <div class="card">
    <div class="card-header">
      <div class="row">
        ...
        <div class="col-lg-1">
          <button
            v-on:click="Object.assign(create_tu, v)"
            class="btn btn-outline-info"
            data-bs-toggle="modal"
            data-bs-target="#ThemTuDo"
          >
            Thêm Tủ Mới
          </button>
        </div>
            ...
            <tbody>
              <tr>
                ...
                <td class="mt-2 text-center">
                  <button
                    v-on:click="changeTrangThai(value)"
                    v-if="value.is_active == 0"
                ...
                <td class="text-center">
                  <button
                    v-on:click="Object.assign(update_tu, value)"
                    class="btn btn-primary m-2"
                    data-bs-toggle="modal"
                    data-bs-target="#uptadetu"
                  >
                    Cập Nhật
                  </button>
                  <button
                    v-on:click="Object.assign(delete_tu, value)"
                    class="btn btn-danger"
                    data-bs-toggle="modal"
                    data-bs-target="#exampleWarningModal"
                  >
                    Xóa
                  </button>
                </td>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <input
            v-model="create_tu.ten_san_pham"
            type="text"
            class="form-control"
            placeholder=""
          />
          ...
          <input
            v-model="create_tu.gia_ban"
            type="text"
            class="form-control"
            placeholder=""
          />
          <label for="">Mã Pin Tủ </label>
          <input
            v-model="create_tu.pin_active"
            type="text"
            class="form-control"
            placeholder=""
          />
          <label for="">Trạng Thái</label>
          <select v-model="create_tu.has_active" class="form-control mt-1">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <label for="">Tên Tủ</label>
          <input
            v-model="update_tu.ten_san_pham"
            type="text"
            class="form-control"
            placeholder=""
          />
            ...
            <div class="d-flex align-items-center">
              ...
              <div class="ms-3">
                ...
                <div class="text-dark">
                  ...
                  <b>{{ delete_tu.ten_san_pham }}</b> này không?
...
export default {
  ...
  methods: {
    ...
    LoadTuDo() {
      baseRequest.get("tu-do/data").then((res) => {
        this.list_tu = res.data.data;
      });
    },
    createTuDo() {
      baseRequest.post("tu-do/create", this.create_tu).then((res) => {
        if (res.data.status) {
    ...
    updateTu() {
      baseRequest.post("tu-do/update", this.update_tu).then((res) => {
        if (res.data.status) {
          toaster.success(res.data.message);
          this.LoadTuDo();
        } else {
    ...
    deleteTu() {
      baseRequest.delete("tu-do/delete/" + this.delete_tu.id).then((res) => {
        if (res.data.status) {
    ...
    changeTrangThai(value) {
      baseRequest.post("tu-do/change-status", value).then((res) => {
        if (res.data.status) {
          toaster.success("Thông báo<br>" + res.data.message);
          this.LoadTuDo();
        } else {
